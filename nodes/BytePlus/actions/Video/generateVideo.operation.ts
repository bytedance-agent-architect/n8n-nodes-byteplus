...
export const description: INodeProperties[] = [
  {
    ...
    options: [
      {
        name: "Seedance 1.0 Pro Fast (Default)",
        value: "seedance-1-0-pro-fast-251015",
      },
  ...
  },
  {
    ...
    description:
      "URL of the last frame image (only available for Seedance 1.0 Pro and 1.5 Pro)",
  },
  {
    displayName: "Prompt",
    name: "prompt",
...
export async function execute(
  ...
  const generateAudio = this.getNodeParameter(
    "generateAudio",
    index,
    false,
  ) as boolean;
  const resolution = this.getNodeParameter(
    "resolution",
    index,
    "720p",
  ) as string;
  ...
  const duration = Math.min(
    maxDuration,
    Math.max(minDuration, Math.round(durationRaw)),
  );
  ...
  if (!taskId) {
    throw new NodeOperationError(
      this.getNode(),
      "No task_id returned from video generation API",
      {
        itemIndex: index,
      },
    );
  ...
  for (;;) {
    ...
    if (["failed", "error", "canceled"].includes(status)) {
      throw new NodeApiError(this.getNode(), pollResponse as JsonObject, {
        itemIndex: index,
        message: `Video generation task failed with status "${status}"`,
      });
    }
